...
SQ *sq_create() {
    ...
    q->the_queue = dlst_create();
    q->buzzer_bucket = dlst_create();
...
void sq_free(SQ *q) {
    ...
    dlst_free(q->the_queue);
    dlst_free(q->buzzer_bucket);
    //free(q->arr2ptr);
...
*/
int sq_length(SQ *q) {
    return dlst_length(q->the_queue);
}
...
int sq_give_buzzer(SQ *q) {
    ...
    if (!dlst_is_empty(q->buzzer_bucket)) {
        buzzer = dlst_pop_front(q->buzzer_bucket);
        q->arr2ptr[buzzer] = dlst_push_back(q->the_queue, buzzer);
        return buzzer;
    } else {
        /*  invariant:
...
int sq_seat(SQ *q) {
    ...
    if (dlst_is_empty(q->the_queue))
        return -1;
    else {
        buzzer = dlst_pop_front(q->the_queue);
        q->arr2ptr[buzzer] = dlst_push_front(q->buzzer_bucket, buzzer);
        q->arr2ptr[buzzer] = NULL;
        return buzzer;
    }
}
...
int sq_kick_out(SQ *q, int buzzer) {
    ...
    if (dlst_remove_first(q->the_queue, q->arr2ptr[buzzer])) {
        q->arr2ptr[buzzer] = dlst_push_front(q->buzzer_bucket, buzzer);
        q->arr2ptr[buzzer] = NULL;
        return 1;
    } else
...
int sq_take_bribe(SQ *q, int buzzer) {
    ...
    if (dlst_remove_first(q->the_queue, q->arr2ptr[buzzer])) {
        q->arr2ptr[buzzer] = dlst_push_front(q->the_queue, buzzer);
        return 1;
    } else {
